...
public class Organization
{
    ...
    // Factory Method: Create Organization WITH default Docker Socket Environment
    public static Organization Create(
        string id,
        string name,
        string defaultEnvironmentId,
        string defaultEnvironmentName,
        string dockerSocketPath = "unix:///var/run/docker.sock")
    {
        ...
        // Create default Docker Socket environment (v0.4 only supports this type)
        var defaultEnvironment = DockerSocketEnvironment.Create(
    ...
    public DockerSocketEnvironment AddDockerSocketEnvironment(string id, string name, string socketPath)
    {
...
- Must have at least one environment at all times
- Exactly one environment must be marked as default
...
public class OrganizationRepository : IOrganizationRepository
{
    ...
    {
        // Find default environment (already deserialized to correct type by EnvironmentJsonConverter)
        var defaultEnv = dto.Environments.First(e => e.IsDefault);

        // Create organization with default Docker Socket environment
        // (v0.4 only supports DockerSocketEnvironment, so we can safely cast)
        if (defaultEnv is not DockerSocketEnvironment defaultSocketEnv)
            throw new InvalidOperationException(
                $"Default environment must be DockerSocketEnvironment in v0.4. Found: {defaultEnv.GetTypeName()}");
        ...
        foreach (var env in dto.Environments.Where(e => !e.IsDefault))
        {
            ...
            {
                org.AddDockerSocketEnvironment(socketEnv.Id.Value, socketEnv.Name, socketEnv.SocketPath);
            }
            else
            {
