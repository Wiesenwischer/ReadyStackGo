...
- Click on stack → Navigate to DeploymentDetail
...
- If container start was successful → new containers are already active, rollback would create inconsistent state
...
{
  "sessionId": "rollback-myapp-1234567890"  // Optional: For real-time progress via SignalR
}
...
Both upgrade and rollback operations support real-time progress tracking via SignalR:

1. **Client subscribes** to the deployment hub with a session ID before calling the API
2. **API receives** the session ID in the request body
3. **Backend sends** progress notifications to the SignalR group `deployment:{sessionId}`

**SignalR Events:**
- `DeploymentProgress` - Sent during operation with phase, percentage, and service info
- `DeploymentCompleted` - Sent when operation succeeds
- `DeploymentFailed` - Sent when operation fails

**Progress Phases:**
- `Planning` - Preparing deployment plan
- `Initializing` - Setting up deployment
- `Network` - Creating/configuring networks
- `RemovingOldContainers` - Stopping and removing old containers
- `PullingImages` - Downloading container images
- `StartingServices` - Starting new containers
- `Persisting` - Saving deployment state
...
- **Partial Rollback** (only individual services) → Post v1.0
