...
import { useRestartProductStore } from '@rsgo/core';
...
export default function RestartProduct() {
  ...
  const store = useRestartProductStore(token, activeEnvironment?.id, productDeploymentId);
  ...
  if (store.state === 'success') {
    const successCount = store.restartResults.filter(r => r.success).length;
    const totalStopped = store.restartResults.reduce((sum, r) => sum + r.containersStopped, 0);
    const totalStarted = store.restartResults.reduce((sum, r) => sum + r.containersStarted, 0);
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-8 dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex flex-col items-center py-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-2">
              {store.deployment?.productDisplayName} ({successCount} stack{successCount !== 1 ? 's' : ''}) containers have been restarted
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
              {totalStopped} stopped, {totalStarted} started
            </p>

            {store.restartResults.length > 0 && (
              <div className="w-full max-w-lg mb-6">
                <div className="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                  {store.restartResults.map((result) => (
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-8 dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex flex-col items-center py-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Restarting {store.deployment?.productDisplayName} in {activeEnvironment?.name}
            </p>
  ...
  if (store.state === 'error' && store.deployment) {
    const successCount = store.restartResults.filter(r => r.success).length;
    const failedCount = store.restartResults.filter(r => !r.success).length;
      ...
      <div className="mx-auto max-w-screen-xl p-4 md:p-6 2xl:p-10">
        ...
        <div className="rounded-2xl border border-red-200 bg-white p-8 dark:border-red-800 dark:bg-white/[0.03]">
          <div className="flex flex-col items-center py-4">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-2">{store.error}</p>
            {store.restartResults.length > 0 && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {successCount} restarted, {failedCount} failed of {store.restartResults.length} stacks
              </p>
            )}
                    ...
                    <div key={result.stackName} className="px-4 py-3 border-b last:border-b-0 border-gray-200 dark:border-gray-700">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`text-sm font-medium ${result.success ? 'text-gray-900 dark:text-white' : 'text-red-600 dark:text-red-400'}`}>
                            {result.stackName}
                          </span>
                        </div>
                        <span className="text-xs text-gray-500 dark:text-gray-400">
                      ...
                      {result.error && (
                        <p className="mt-1 ml-7 text-xs text-red-600 dark:text-red-400">{result.error}</p>
                      )}
                    </div>
                  ))}
                </div>
      ...
      <div className="rounded-2xl border border-amber-200 bg-white p-8 dark:border-amber-800 dark:bg-white/[0.03]">
        <div className="flex flex-col items-center py-4">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-2 text-center">
            Are you sure you want to restart all containers of <strong className="text-gray-900 dark:text-white">{store.deployment?.productDisplayName}</strong>?
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 text-center max-w-md">
            This will stop and restart all {store.deployment?.totalStacks} stack{(store.deployment?.totalStacks ?? 0) !== 1 ? 's' : ''} with {store.totalServices} container{store.totalServices !== 1 ? 's' : ''} sequentially.
                ...
                .map((stack) => (
                  <div key={stack.stackName} className="flex items-center justify-between py-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-700 dark:text-gray-300">{stack.stackDisplayName}</span>
                      {stack.deploymentStackName && (
                        <span className="text-xs text-gray-400 dark:text-gray-500 font-mono">({stack.deploymentStackName})</span>
                      )}
                    </div>
                    <span className="text-xs text-gray-500 dark:text-gray-400">
                      {stack.serviceCount} service{stack.serviceCount !== 1 ? 's' : ''}
