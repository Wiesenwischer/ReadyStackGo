    ...
    <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
      ...
      {store.success && (
        <div className="mb-6 rounded-md bg-green-50 p-4 dark:bg-green-900/20">
          <div className="flex justify-between items-start">
            <p className="text-sm text-green-800 dark:text-green-200">{store.success}</p>
            <button onClick={store.clearSuccess} className="text-green-500 hover:text-green-600">
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
              </svg>
            </button>
      ...
      {store.loading ? (
        ...
        <div className="space-y-6">
            ...
            <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                ...
                <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${
                  store.config?.certificateInfo?.isExpired
                    ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
                    : store.config?.certificateInfo?.isExpiringSoon
                ...
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <dd className="mt-1 text-sm font-medium text-gray-900 dark:text-white flex items-center gap-2">
                      {store.config.mode}
                      {store.config.certificateInfo.isSelfSigned && (
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm font-mono text-gray-900 dark:text-white">
                      {store.config.certificateInfo.subject}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-sm text-gray-500 dark:text-gray-400">Issuer</dt>
                    <dd className="mt-1 text-sm font-mono text-gray-900 dark:text-white">
                      {store.config.certificateInfo.issuer}
                    </dd>
                  </div>
                  <div>
                    ...
                    <dd className={`mt-1 text-sm font-medium ${
                      ...
                      {store.formatDate(store.config.certificateInfo.expiresAt)}
                    </dd>
                  </div>
                  <div className="sm:col-span-2">
                    ...
                    <dd className="mt-1 text-xs font-mono text-gray-600 dark:text-gray-400 break-all">
                      {store.config.certificateInfo.thumbprint}
                    </dd>
                  </div>
                </dl>
              ) : (
          ...
          {store.showCertificateSettings && store.leStatus?.isConfigured && (
            <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
              <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
              ...
              <div className="px-6 py-5">
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <dd className="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                      {store.leStatus.domains.join(", ")}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-sm text-gray-500 dark:text-gray-400">Challenge Type</dt>
                    <dd className="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                      {store.leStatus.challengeType}
                    </dd>
                  </div>
                  {store.leStatus.certificateExpiresAt && (
                    <div>
                      <dt className="text-sm text-gray-500 dark:text-gray-400">Certificate Expires</dt>
                      <dd className="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                        {store.formatDate(store.leStatus.certificateExpiresAt)}
                      </dd>
                    </div>
                  )}
                  {store.leStatus.isUsingStaging && (
                    <div className="sm:col-span-2">
                ...
                {store.leStatus.lastError && (
                  <div className="mt-4 rounded-md bg-red-50 p-3 dark:bg-red-900/20">
                    <p className="text-sm text-red-700 dark:text-red-300">
                      Last error: {store.leStatus.lastError}
                    </p>
                  </div>
                )}
              </div>
          ...
          {store.showCertificateSettings && store.leStatus?.pendingDnsChallenges && store.leStatus.pendingDnsChallenges.length > 0 && (
            <div className="rounded-2xl border border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20">
              ...
              <div className="px-6 py-5 space-y-3">
                {store.leStatus.pendingDnsChallenges.map((challenge, idx) => (
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed ${
                    store.config?.httpEnabled ? "bg-brand-600" : "bg-gray-200 dark:bg-gray-700"
                  }`}
                  role="switch"
                  aria-checked={store.config?.httpEnabled}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                      store.config?.httpEnabled ? "translate-x-5" : "translate-x-0"
                    }`}
                  />
                </button>
              </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    {store.config?.reverseProxy?.enabled
                      ? "Running behind a reverse proxy (nginx, Traefik, etc.)"
                      : "Direct connection mode (no reverse proxy)"}
                  </p>
                </div>
                <button
                  onClick={store.handleReverseProxyToggle}
                  disabled={store.actionLoading}
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed ${
                    store.config?.reverseProxy?.enabled ? "bg-brand-600" : "bg-gray-200 dark:bg-gray-700"
                  }`}
                  role="switch"
                  aria-checked={store.config?.reverseProxy?.enabled}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                      store.config?.reverseProxy?.enabled ? "translate-x-5" : "translate-x-0"
                    }`}
                  />
                </button>
              </div>
            ...
            {store.config?.reverseProxy?.enabled && (
              <div className="px-6 py-5">
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-3">
                    <label className={`flex items-start p-3 rounded-lg border cursor-pointer transition-colors ${
                      store.config.reverseProxy.sslMode === "Termination"
                      ...
                      <input
                        ...
                        name="sslMode"
                        value="Termination"
                        checked={store.config.reverseProxy.sslMode === "Termination"}
                        onChange={() => store.handleSslModeChange("Termination")}
                        disabled={store.actionLoading}
                    ...
                    <label className={`flex items-start p-3 rounded-lg border cursor-pointer transition-colors ${
                      store.config.reverseProxy.sslMode === "Passthrough"
                        ? "border-brand-500 bg-brand-50 dark:bg-brand-900/20"
                        : "border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600"
                    }`}>
                      <input
                        ...
                        name="sslMode"
                        value="Passthrough"
                        checked={store.config.reverseProxy.sslMode === "Passthrough"}
                        onChange={() => store.handleSslModeChange("Passthrough")}
                        disabled={store.actionLoading}
                        className="mt-0.5 h-4 w-4 text-brand-600 border-gray-300 focus:ring-brand-500"
                      />
                      <div className="ml-3">
                    ...
                    <label className={`flex items-start p-3 rounded-lg border cursor-pointer transition-colors ${
                      store.config.reverseProxy.sslMode === "ReEncryption"
                        ? "border-brand-500 bg-brand-50 dark:bg-brand-900/20"
                        : "border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600"
                    }`}>
                      <input
                        ...
                        name="sslMode"
                        value="ReEncryption"
                        checked={store.config.reverseProxy.sslMode === "ReEncryption"}
                        onChange={() => store.handleSslModeChange("ReEncryption")}
                        disabled={store.actionLoading}
                        className="mt-0.5 h-4 w-4 text-brand-600 border-gray-300 focus:ring-brand-500"
                      />
                      <div className="ml-3">
                    ...
                    <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mb-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2 text-sm">
                          <svg className={`w-4 h-4 ${store.config.reverseProxy.trustForwardedFor ? "text-green-500" : "text-gray-400"}`} fill="currentColor" viewBox="0 0 20 20">
                            {store.config.reverseProxy.trustForwardedFor ? (
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                            ) : (
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                            )}
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          <svg className={`w-4 h-4 ${store.config.reverseProxy.trustForwardedProto ? "text-green-500" : "text-gray-400"}`} fill="currentColor" viewBox="0 0 20 20">
                            {store.config.reverseProxy.trustForwardedProto ? (
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                            ) : (
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                            )}
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          <svg className={`w-4 h-4 ${store.config.reverseProxy.trustForwardedHost ? "text-green-500" : "text-gray-400"}`} fill="currentColor" viewBox="0 0 20 20">
                            {store.config.reverseProxy.trustForwardedHost ? (
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                            ) : (
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                            )}
                ...
                {store.config.reverseProxy.pathBase && (
                  <div className="flex items-center gap-2 text-sm mb-4">
                    <svg className="w-4 h-4 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                    </svg>
                    <span className="text-gray-700 dark:text-gray-300">
                      Path Base: <code className="text-xs bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded">{store.config.reverseProxy.pathBase}</code>
                    </span>
                  </div>
                )}
