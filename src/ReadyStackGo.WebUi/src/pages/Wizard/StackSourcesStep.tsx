...
export default function StackSourcesStep({ onNext }: StackSourcesStepProps) {
  const [sources, setSources] = useState<WizardRegistrySource[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      await onNext(Array.from(selectedIds));
    } catch (err) {
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          ...
          These provide ready-to-deploy stack definitions from the community.
          You can add or remove sources later in Settings.
        </p>
      </div>
