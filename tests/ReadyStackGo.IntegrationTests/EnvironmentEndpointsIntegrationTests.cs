...
public class EnvironmentEndpointsIntegrationTests : AuthenticatedTestBase
{
    ...
    public async Task POST_CreateEnvironment_WithValidData_ReturnsSuccess()
    {
        ...
        // v0.6: DDD domain model uses enum-style type name
        result.Environment.Type.Should().Be("DockerSocket");
        result.Environment.ConnectionString.Should().Be("/var/run/docker.sock");
        // v0.6: ID is generated by the service, not provided by the client
        result.Environment.Id.Should().NotBeNullOrEmpty();
    }
    ...
    [Fact]
    public async Task POST_CreateEnvironment_WithDuplicateName_ReturnsError()
    {
        // Arrange - Create first environment
        // Note: v0.6 generates IDs server-side, so we test duplicate name instead
        var uniqueName = $"Duplicate Test {Guid.NewGuid():N}";
        var request = new
        {
            name = uniqueName,
            socketPath = "/var/run/docker.sock"
        ...
        {
            name = uniqueName, // Same name - should fail
            socketPath = "/var/run/docker.sock"
        };
        var secondResponse = await Client.PostAsJsonAsync("/api/environments", duplicateRequest);

        // Assert
        // v0.6: The error might be returned as Success=false with message or as 400 Bad Request
        if (secondResponse.StatusCode == HttpStatusCode.OK)
        {
            ...
            result!.Success.Should().BeFalse("Duplicate name should be rejected");
    ...
    public async Task EnvironmentSetupFlow_AfterWizard_WorksCorrectly()
    {
        ...
        // v0.6: ID is generated by the service, verify it's a valid GUID
        created.Environment!.Id.Should().NotBeNullOrEmpty();
        Guid.TryParse(created.Environment.Id, out _).Should().BeTrue("ID should be a valid GUID");
        created.Environment.Name.Should().Be("Setup Flow Environment");
        // v0.6: Type uses enum name format
        created.Environment.Type.Should().Be("DockerSocket");
        ...
        finalList.Environments.Should().Contain(e => e.Id == created.Environment.Id);
    }
